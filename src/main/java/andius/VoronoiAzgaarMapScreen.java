...
public class VoronoiAzgaarMapScreen extends BaseScreen {
    ...
    public VoronoiAzgaarMapScreen() {
        ...
        moongateMap.put(Moongate.GATE_0, renderer.getCenter(6018).cell);
        moongateMap.put(Moongate.GATE_1, renderer.getCenter(2216).cell);
        moongateMap.put(Moongate.GATE_2, renderer.getCenter(1770).cell);
        moongateMap.put(Moongate.GATE_3, renderer.getCenter(695).cell);
        moongateMap.put(Moongate.GATE_4, renderer.getCenter(5199).cell);
        moongateMap.put(Moongate.GATE_5, renderer.getCenter(5712).cell);
        moongateMap.put(Moongate.GATE_6, renderer.getCenter(8541).cell);
        moongateMap.put(Moongate.GATE_7, renderer.getCenter(567).cell);
    }
    ...
    public boolean keyUp(int keycode) {
        ...
        if (keycode == Input.Keys.UP) {
            ...
            Portal p = this.currentLocation.cell != null ? this.currentLocation.cell.portal : null;
            if (p != null) {
    ...
    public static class Cell {
        ...
        public Color color() {
            if (this.color == null) {
                ...
                if (height >= 70) {
                    // mount icon
                    //this.color = new Color((height) * .01f, (height) * .01f, (height) * .01f, 1);
                    ...
                    // swamp icons
                    this.color = new Color(0x90d092ff);
    ...
    public static Color hslColorToRgb(double hue, double saturation, double lightness) {
        ...
        float red = percToColor(hueToRgb(p, q, hue + oneThird));
        float green = percToColor(hueToRgb(p, q, hue));
        float blue = percToColor(hueToRgb(p, q, hue - oneThird));

        return new Color(Color.rgba8888(red, green, blue, 1f));
    }
    ...
    public class VoronoiAzgaarRenderer implements MapRenderer {
        ...
        public VoronoiAzgaarRenderer(List<Float[]> pts, float factor) {
            ...
            this.bounds = this.voro.get_plotBounds();

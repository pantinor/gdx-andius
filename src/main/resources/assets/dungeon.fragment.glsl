...
// NEW: always separate ambient when we have ambient.
...
void main() {
		...
		gl_FragColor.rgb = diffuse.rgb + emissive.rgb;
            ...
            #if defined(ambientFlag) && defined(separateAmbientFlag)
                #ifdef shadowMapFlag
                    vec3 direct = diffuse.rgb * (getShadow() * v_lightDiffuse);
                #else
                    vec3 direct = diffuse.rgb * v_lightDiffuse;
                #endif //shadowMapFlag

                vec3 ambient = diffuse.rgb * v_ambientLight;

                // Ambient is a floor, not added on top.
                gl_FragColor.rgb = max(ambient, direct) + emissive.rgb;
            #else
                // Fallback if ambient isn't separated for some reason.
                #ifdef shadowMapFlag
                ...
                // Ambient as minimum, specular on top.
                gl_FragColor.rgb = max(ambient, direct) + specular + emissive.rgb;
            #else
                // Fallback if ambient not separated.
                #ifdef shadowMapFlag

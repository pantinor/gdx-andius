...
struct SpotLight {
    ...
    float cutoffAngle;  // in degrees; angle from axis to edge of cone
    float exponent;     // focus: higher = tighter center
};
...
// NEW: always separate ambient when we have ambient at all,
// not only when using shadow maps.
			...
			for (int i = 0; i < numPointLights; i++) {
				...
				v_lightDiffuse += value;
                    ...
                    for (int i = 0; i < numSpotLights; i++) {
                        ...
                        // Vector from light to vertex in world space
                        vec3 lightToVertex = pos.xyz - u_spotLights[i].position;
                        float dist2 = dot(lightToVertex, lightToVertex);

                        // Direction from light to vertex
                        vec3 L = lightToVertex * inversesqrt(max(dist2, 1e-6));

                        // Spotlight cone: compare angle between spotlight axis and light->vertex dir
                        ...
                        if (cosTheta > cosCutoff) {
                            // Focus inside the cone
                            float spotFactor = pow(cosTheta, u_spotLights[i].exponent);

                            // Distance attenuation (matches your point light style)
                            float att = u_spotLights[i].intensity / (1.0 + dist2);

                            // For actual lighting we need vertex->light direction
                            vec3 vertexToLight = -L;
                            ...
                            #ifdef specularFlag
                                float halfDotView = max(0.0, dot(normal,
                                    normalize(vertexToLight + viewVec)));
                                v_lightSpecular += value * pow(halfDotView, u_shininess);
                            #endif
                            }
                        }
